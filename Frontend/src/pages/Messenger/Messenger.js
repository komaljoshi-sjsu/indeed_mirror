...
const Messenger = (props) => {
    ...
    const [userId, setUserId] = useState(parseInt(21)); // add id from store
    ...
    useEffect(() => {
        const getConversations = async () => {
            try {
                ...
                const convResponse = await axios.get("/api/getConversationById/" + userId);
                let removeArr = [];
                ...
                <div className="chatMenuWrapper">
                    ...
                    {conversations.map((c) => (
                        <div onClick={() => setCurrentChat(c)}>
                            <Conversation conversation={c} currentUser={userId} role={role} />
                        </div>
                    ))}
                    ...
                    {currentChat ? (
                        <div>
                            <div className="chatBoxTop">
                                {messages.map((msg) => (
                                    <div ref={scrollRef}>
                                        <Message message={msg} own={Number(msg.sender) === userId} />
                                    </div>
                                ))}
                        ...
                        </div>) : (<span className="noConversationMsg">No conversations yet</span>)}
                </div>
            </div>
        </div>
    );
