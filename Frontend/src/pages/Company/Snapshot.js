...
function Snapshot(props) {
  ...
  const setFounded = bindActionCreators(
    companyActionCreator.setFounded,
    dispatch,
  )
  const setSize = bindActionCreators(companyActionCreator.setSize, dispatch)
  const setRevenue = bindActionCreators(
    companyActionCreator.setRevenue,
    dispatch,
  )
  const setAbout = bindActionCreators(companyActionCreator.setAbout, dispatch)
  const setDescription = bindActionCreators(
    companyActionCreator.setDescription,
    dispatch,
  )
  const setMission = bindActionCreators(
    companyActionCreator.setMission,
    dispatch,
  )
  const setWHScore = bindActionCreators(
    companyActionCreator.setWHScore,
    dispatch,
  )
  const setLScore = bindActionCreators(companyActionCreator.setLScore, dispatch)
  const setApScore = bindActionCreators(
    companyActionCreator.setApScore,
    dispatch,
  )
  const setNoOfReviews = bindActionCreators(
    companyActionCreator.setNoOfReviews,
    dispatch,
  )
  const setCulture = bindActionCreators(
    companyActionCreator.setCulture,
    dispatch,
  )
  const setValues = bindActionCreators(companyActionCreator.setValues, dispatch)
  const setFeaturedReviews = bindActionCreators(
    companyActionCreator.setFeaturedReviews,
    dispatch,
  )
  const setIndustry = bindActionCreators(
    companyActionCreator.setIndustry,
    dispatch,
  )
  useEffect(() => {
    axios
      .get('http://localhost:5000/api/snapshot/' + companyid)
      .then((res) => {
    ...
    axios
      .get('http://localhost:5000/api/featuredReviews/' + companyid)
      .then((res) => {
        ...
        <div className="row">
          ...
          <p>
            <small style={{ color: 'gray' }}>
              Scores based on about {noOfReviews} responses to Indeed's survey
              on work happiness.
            </small>
          </p>
          <div className="col">
            <div>{whScore} Work Happiness Score</div>
            <br></br>
          ...
          <div className="col">
            <Card>
              <Card.Body>
                ...
                <Card.Text>More than {size}</Card.Text>
              </Card.Body>
            </Card>
          </div>
          <div className="col">
            <Card>
              <Card.Body>
                ...
                <Card.Text>{revenue}(USD)</Card.Text>
              </Card.Body>
            </Card>
          </div>
          <div className="col">
          ...
          {reviews.map((review) => {
            ...
            return (
              <div>
                <div>
                  <p>
                    <img src="/images/user.png" height="24px" width="24px" />
                    <small style={{ color: 'gray' }}>
                      {' '}
                      {review.reviewerRole} in {review.city},{review.state}
                    </small>
                  </p>
                  <b>{review.rating} </b>
                  <img src="/images/star.png" height="19px" width="19px" />
                  <span style={{ color: 'gray' }}> on {postedDate}</span>
                  <p>
