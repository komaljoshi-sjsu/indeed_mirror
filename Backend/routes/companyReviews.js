...
router.get('/companyReviewsPaginated', (req, res) => {
  ...
  let sql =
    'SELECT r.*, c.companyName FROM Review r, Company c where r.companyId=' +
    mysql.escape(queryObject.companyId) +
    ' and r.companyId = c.companyId and r.isFeatured=1 and r.adminReviewStatus=? ORDER BY FIELD(jobSeekerId, ?) DESC LIMIT ?,?'
  console.log(sql)
  connection.query(
    sql,
    [adminReviewStatus, queryObject.jobSeekerId, offset, limit],
    (err, results) => {
      if (err) {
        res.writeHead(401, {
...
router.get('/companyReviewsRatingSort', (req, res) => {
  ...
  let sql =
    'SELECT r.*, c.companyName FROM Review r, Company c where r.companyId=' +
    mysql.escape(queryObject.companyId) +
    ' and r.companyId = c.companyId and r.isFeatured=1 and r.adminReviewStatus=? ORDER BY rating DESC LIMIT ?,?'
  console.log(sql)
...
router.get('/companyReviewsDateSort', (req, res) => {
  ...
  let sql =
    'SELECT r.*, c.companyName FROM Review r, Company c where r.companyId=' +
    mysql.escape(queryObject.companyId) +
    ' and r.companyId = c.companyId and r.isFeatured=1 and r.adminReviewStatus=? ORDER BY postedDate DESC LIMIT ?,?'
  console.log(sql)
  connection.query(sql, [adminReviewStatus, offset, limit], (err, results) => {
    if (err) {
      res.writeHead(401, {
...
router.get('/companyReviewsHelpfulSort', (req, res) => {
  ...
  let sql =
    'SELECT r.*, c.companyName FROM Review r, Company c where r.companyId=' +
    mysql.escape(queryObject.companyId) +
    ' and r.companyId = c.companyId and r.isFeatured=1 and r.adminReviewStatus=? ORDER BY yesReviewHelpfulCount DESC LIMIT ?,?'
  console.log(sql)
  connection.query(sql, [adminReviewStatus, offset, limit], (err, results) => {
    if (err) {
      res.writeHead(401, {
...
router.get('/companyReviews', (req, res) => {
  ...
  const adminReviewStatus = 'APPROVED'
  let sql =
    'SELECT r.*, c.companyName FROM Review1 r, Company c where r.companyId=' +
    mysql.escape(queryObject.companyId) +
    ' and r.companyId = c.companyId and r.isFeatured=1 and r.adminReviewStatus=?'
  console.log(sql)
...
router.post('/updateHelpfulCount', (req, res) => {
  ...
  let sql =
    'UPDATE Review SET yesReviewHelpfulCount = ?, noHelpfulCount = ? WHERE reviewId = ?'
  let data = [
    req.body.yesReviewHelpfulCount,
    req.body.noHelpfulCount,
    req.body.reviewId,
  ]
...
router.post('/saveReview', (req, res) => {
  ...
  let sql =
    'INSERT INTO Review(reviewTitle, reviewerRole, city, state, postedDate, rating, workHappinessScore, learningScore, appraisalScore, reviewComments, pros, cons, ceoApprovalRating, howToPrepare, noHelpfulCount, yesReviewHelpfulCount, isFeatured, adminReviewStatus, jobSeekerId, companyId ) VALUES (?,?,?,?,now(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) '
  let data = [
    ...
    req.body.state,
    req.body.rating,
    req.body.workHappinessScore,
    req.body.learningScore,
    req.body.appreciationScore,
    req.body.reviewComments,
    req.body.pros,
    req.body.cons,
    req.body.ceoApprovalRating,
    req.body.howToPrepare,
    0,
    0,
    0,
    'PENDING_APPROVAL',
  ...
  connection.query(sql, data, (err, results) => {
    if (err) {
      ...
      let sql =
        'select round(avg(Rating),1) as avgRating, round(avg(workHappinessScore),1) as avgWHScore, round(avg(learningScore),1) as avgLScore, round(avg(appraisalScore),1) as avgAppScore, round(avg(ceoApprovalRating),1) as avgCeoScore, count(Rating) as totalReviews from Review where companyId=?'
