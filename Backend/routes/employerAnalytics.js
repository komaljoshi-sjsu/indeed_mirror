...
router.get('/jobPosted', (req, res) => {
  try {
    ...
    conn.query('SELECT count(Job.jobId) as countJobId ,Employer.name from Job,Employer where Employer.id=Job.id  and Job.id =? and YEAR(Job.jobPostedDate) <=YEAR(CURDATE()) and YEAR(Job.jobPostedDate) >= YEAR(CURDATE())-1;', async function (err, results) {
      if (results.length <= 0) {
