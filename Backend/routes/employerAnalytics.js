...
router.get('/jobPosted', (req, res) => {
  try {
    ...
    conn.query('SELECT count(Employer.companyId) as countCompId ,Employer.name from Job,Employer where Employer.id=1 and Job.companyId=Employer.companyId and YEAR(Job.jobPostedDate) <=YEAR(CURDATE()) and YEAR(Job.jobPostedDate) >= YEAR(CURDATE())-1;', async function (err, results) {
      if (results.length <= 0) {
